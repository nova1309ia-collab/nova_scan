...
function writeLS(params){{
  /* localStorage est partagé entre tous les iframes same-origin sur Streamlit */
  try{{
    for(const[k,v] of Object.entries(params))
      localStorage.setItem('nova_qp_'+k, v);
  }}catch(e){{
    /* Si localStorage bloqué (rare), fallback postMessage vers parent */
    try{{ window.parent.postMessage({{type:'nova_qp',params:params}},'*'); }}catch(e2){{}}
  }}
}}
function doCrop(){{
  ...
  writeLS({{'{qp_coins_key}':JSON.stringify(disp),'{qp_crop_key}':'1'}});
}}
function doSkip(){{
  writeLS({{'{qp_crop_key}':'skip'}});
}}
...
def flux_image(img_pil, nom_pdf, prefix):
    ...
    if state == "canvas":
        ...
        # ── Pont de polling localStorage → query_params (hauteur 0, invisible) ──
        # Ce components.html tourne dans un iframe same-origin et peut lire
        # localStorage écrit par le canvas. Dès qu'il détecte les clés nova_qp_*
        # il navigue window.parent vers l'URL avec query_params, déclenchant le rerun.
        components.html(f"""<!DOCTYPE html><html><body><script>
(function poll(){{
  var crop=localStorage.getItem('nova_qp_{qp_crop_key}');
  if(!crop){{ setTimeout(poll,300); return; }}
  localStorage.removeItem('nova_qp_{qp_crop_key}');
  var coins=localStorage.getItem('nova_qp_{qp_coins_key}')||'';
  localStorage.removeItem('nova_qp_{qp_coins_key}');
  try{{
    var url=new URL(window.parent.location.href);
    url.searchParams.set('{qp_crop_key}',crop);
    if(coins) url.searchParams.set('{qp_coins_key}',coins);
